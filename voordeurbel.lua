...
return {
	...
	execute = function(domoticz, device)
		...
		debug = false			
		domoticz.helpers.sendnotification(domoticz,'De deurbel gaat','Er staat iemand voor de deur, misschien open doen?',schemerlamp_deur.state,lamp_spoelb_keuken.state,lamp_boven_tv.state,schemerlamp_bank.state,lamp_hal_boven.state,1,2,1)
		if (IsDark == 'On') then
