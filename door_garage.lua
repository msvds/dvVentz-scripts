...
return {
	...
	execute = function(domoticz, device)
		...
		if (domoticz.devices('Garage deur').state == 'Open' and domoticz.devices('Garage deur').lastUpdate.minutesAgo > 30) then
			if ((domoticz.globalData.OpenCDeurgarage > Open_timeout_outside and domoticz.globalData.door_garage_message_interval > message_interval) or (test == true and domoticz.devices('Test Switch').state == 'On')) then
				...
				domoticz.globalData.OpenCDeurgarage = 0
			end
		end
	end
}
