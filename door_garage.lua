...
return {
	...
	execute = function(domoticz, device)
		...
		local garagedoor = domoticz.devices['Garage deur']
		debug = false
		if (garagedoor.state == 'Open' and door.lastUpdate.minutesAgo > 30) then
			if ((domoticz.globalData.OpenC_Deurgarage > Open_timeout_outside and domoticz.globalData.door_garage_message_interval > message_interval) or (test == true and test_switch.state == 'On')) then
				sendnotification('Garage deur alarm','De garage deur is voor meer dan 30 minuten open!',schemerlamp_deur.state,lamp_spoelb_keuken.state,lamp_boven_tv.state,schemerlamp_bank.state,lamp_hal_boven.state,3,3,5)
