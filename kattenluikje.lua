	...
	execute = function(domoticz, device)
		...
		if (domoticz.time.matchesRule('at 9:59-10:01')) then
			if (domoticz.data.counter == 0) then
				...
				domoticz.log('De laatste tijd is het kattenluikje niet gebruikt, check even of alles goed werkt',domoticz.LOG_INFO)
			else
				...
				domoticz.log('Het kattenluikje is de afgelopen 24 uur ' ..domoticz.data.counter .. 'x gebruikt.',domoticz.LOG_INFO)
			end
